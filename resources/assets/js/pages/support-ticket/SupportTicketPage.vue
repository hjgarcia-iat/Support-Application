<template>
    <div class="p-8 bg-gray-100 border">
        ...
        <form method="POST" enctype="multipart/form-data" @submit.prevent="submitForm">
            ...
            <div v-show="step === 1">
                <div class="mb-6">
                    ...
                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_1" value="Curriculum" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_1"><strong>Curriculum Usage</strong> &mdash; Questions about how our curriculum is to be used in the classroom or where to find curriculum resources.</label>
                    </div>
                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_2" value="Errata" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_2"><strong>Curriculum Error</strong> &mdash; Curriculum issues such as typos, incorrect label, or factual correctness.</label>
                    </div>

                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_3" value="Integration" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_3"><strong>Integration Issue</strong> &mdash; Rostering or login issues related to integrations with Clever, Canvas, Schoology, Google Classroom, etc.</label>
                    </div>

                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_4" value="Operations" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_4"><strong>Coupons, Specimens, Kits</strong> &mdash; Redeem coupons, report missing or damaged materials.</label>
                    </div>

                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_5" value="Product" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_5"><strong>Product Usage</strong> &mdash; Questions like how do I do this, Where do I find that. But not about the curriculum.</label>
                    </div>


                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_8" value="Feedback" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_8"><strong>Feedback/Feature Request</strong> &mdash; Ideas for improving our products or providing feedback about our products and services.</label>
                    </div>

                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_9" value="Order Status" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_9"><strong>Order/Shipment Status</strong> &mdash; Inquire about the status of an existing order or shipment.</label>
                    </div>

                    <div class="flex items-center">
                        <input type="radio" name="reason" id="reason_7" value="Other Issue" v-model="reason"> <label
                        class="ml-2 text-grey-darker text-sm mb-2"
                        for="reason_7"><strong>Other Issue</strong> &mdash; Something is not working as it should, forgotten password, or other issue not listed above.</label>
                    </div>
            ...
            <div v-show="step === 2">
                <div class="mb-6">
                    <label for="name" class="block text-grey-darker text-sm font-bold mb-2"> <small
                ...
                <div class="mb-6">
                    <label for="district" class="block text-grey-darker text-sm font-bold mb-2"> <small
                ...
                <div class="mb-6">
                    <label for="subject" class="block text-grey-darker text-sm font-bold mb-2"> <small
                ...
                <div class="mb-6">
                    <label for="details" class="block text-grey-darker text-sm font-bold mb-2"><small
                        class="text-lg text-red-600">*</small> Details</label>

                    <textarea name="details" id="details"
                              class="appearance-none block w-full border border-gray-500 text-grey-darker border py-3 px-4 mb-3 focus:outline-none focus:border-orange-500"
                              cols="30" placeholder="Details" rows="5" v-model="details"></textarea>
                    <form-error :error=formErrors.details[0] v-if="formErrors.details"></form-error>
                </div>


                <div class="mb-6">
                    <label class="block text-grey-darker text-sm font-bold mb-2" for="files">Files</label>
                    <vue-dropzone ref="dropzone" @vdropzone-sending="sendingFiles" @vdropzone-file-added="addedFile"
                                  @vdropzone-complete="uploadComplete" id="files" v-model="files"
                    ...
                    <div class="flex justify-between items-center text-gray-500">
                        <span>Accepted file type: jpg, png, gif, doc, pdf</span>
                    </div>

                    <form-error :error=formErrors.file[0] v-if="formErrors.file"></form-error>
                </div>
            </div>
            <div class="flex mb-4 items-center justify-between">
                <div v-if="step===1" class="flex items-center w-full">
                    <button @click.prevent="step++" type="submit" :disabled="reason===''"
