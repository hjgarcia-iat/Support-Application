                    ...
                    <form method="POST"
                            ...
                            <div class="block mt-1 w-full col-span-5 flex items-center">
                                <div>
                                    ...
                                    <input id="pinned-1" type="radio"
                                           {{ old('pinned', $article->pinned) === true ? 'checked' : '' }}
                                ...
                                <div>
                                    ...
                                    <input id="pinned-0" type="radio" {{ old('pinned', $article->pinned) === false ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                           name="pinned" value="0">
                                </div>
                                ...
                                <x-select id="name"
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ (collect(old('categories', $article->categories->contains($category->id)))->contains($category->id)) ? 'selected':'' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </x-select>
                        ...
                        <div class="mt-4 md:grid grid-cols-6 gap-8">
                            ...
                            <div class="col-span-5">
                                <x-textarea id="post" class="block mt-1 w-full h-48" name="content" :value="old('content')">{{ old('content', $article->content) }}</x-textarea>
                            </div>
                        </div>
